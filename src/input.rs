...
impl JoystickState {
    ...
    pub fn update(&mut self) {
        ...
        if new_gamepads {
                ...
                .filter(|(id, _)| !gp.iter().any(|gp| gp.gid == *id))
            {
                ...
                let msg = Message::JoystickUpdate {
                    removed: false,
    ...
    fn apply_joystick_safety(&self) {
        let msg = Message::UpdateEnableStatus {
            enabled: false,
            from_backend: true,
        };
        self.addr.do_send(msg);
...
pub fn joystick_callback() -> Vec<Vec<JoystickValue>> {
    ...
    sorted_joysticks.sort_by(|a, b| {
        mappings
            .get(&state.map_gid(a.id()).unwrap())
            .unwrap_or(&0)
            .cmp(mappings.get(&state.map_gid(b.id()).unwrap()).unwrap_or(&1))
    });
